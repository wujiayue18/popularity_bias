...
class Loader(BasicDataset):
    ...
    def __init__(self,config = world.config,path="../data/gowalla"):
        ...
        self.users_D = np.array(self.UserItemNet.sum(axis=1)).squeeze() #[n_users,]每个用户的du
        # print(f"users_D:{self.users_D}")
        # print(f"users_D count: {Counter(self.users_D)}")
        self.users_D[self.users_D == 0.] = 1 
        self.items_D = np.array(self.UserItemNet.sum(axis=0)).squeeze() #[m_item,]每个item的du
    ...
    def item_popularity_label_threshold(self):
        # labels = np.abs(self.high_threshold - self.items_D) 
