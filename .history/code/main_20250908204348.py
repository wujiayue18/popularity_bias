...
# weight_file_base = f"./checkpoints/{world.dataset}/mf-{world.dataset}-64-mf.pth.tar"
...
#加载模型
    try:
        if world.config['steer_train']:
            ...
            # world.cprint(f"loaded steer model weights from {weight_file_steer}")
            Recmodel.rec_model.load_state_dict(torch.load(weight_file_base,map_location=torch.device('cpu')))
            # Recmodel.load_state_dict(torch.load(weight_file_steer,map_location=torch.device('cpu')))
        else:
