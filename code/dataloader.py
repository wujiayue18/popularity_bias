...
class Loader(BasicDataset):
    ...
    def __init__(self,config = world.config,path="../data/gowalla"):
        ...
        self.top_items, self.tail_items = self._popularity_tail_items()
        self.testtailDict = self.__build_tail_test()
        self.highpo_samples, self.lowpo_samples = self._popularity_samples()  
        self.item_popularity_labels = self.item_popularity_label() 
        self.threshold_item_popularity_label = self.item_popularity_label_threshold()
        if world.config['load'] == 0:
    ...
    def _popularity_tail_items(self):
        ...
        top_items = sorted_indices[:top_20_percent_count]  
        tail_items = sorted_indices[top_20_percent_count:]  
    ...
    def item_popularity_label_threshold(self):
        adjustments = np.zeros_like(self.items_D, dtype=float)  
        ...
        adjustments[high_mask] = np.sqrt(adjustments[high_mask])  
        low_mask = self.items_D <= self.low_threshold
        adjustments[low_mask] = (self.low_threshold - self.items_D[low_mask]) / self.low_threshold 
        adjustments[low_mask] = np.sqrt(adjustments[low_mask]) 
        adjustments = adjustments * world.config['epsilon']
        return adjustments
